class VirtualMachine(object):
    ...
    #Constructor
    def __init__(self, vm_type, vm_id, memory, cpu_count, io, price, state, migration_time, boot_time,
            max_users):
    ...
    def print_configuration(self):
        print "VM type : %s \nVM ID : %s\nMemory : %s \nNo of CPUs : %s \nIO : %s \nPrice : %s\n \
            State : %s\nMigration time : %s\nBoot time : %s\nMax users : %s" \
            %(self.vm_type, self.vm_id, self.memory, self.cpu_count, self.io, self.price,
